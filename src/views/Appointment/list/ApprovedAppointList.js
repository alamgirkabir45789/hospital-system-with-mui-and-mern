...
const ApprovedAppointList = () => {
  ...
  const getAllPatient = async () => {
    const res = await axios.get(`http://localhost:7000/api/appointment`, { params: { isApproved: true } });
  ...
  const handleVisit = async (e, patient) => {
    const patientId = patient._id;
    const payload = { ...patient, isVisited: true };
          ...
          <TableBody>
            {patients?.map((ul, index) => (
              <TableRow key={ul._id}>
                ...
                <TableCell align="center">{ul.isApproved ? 'Approved' : 'Visited'}</TableCell>
                <TableCell align="center">
                  {
                    <Checkbox
                      variant="outlined"
                      name="isActive"
                      checked={ul?.isVisited}
                      onChange={e => handleVisit(e, ul)}
                    />
                  }
                </TableCell>
              </TableRow>
...
export default ApprovedAppointList;
